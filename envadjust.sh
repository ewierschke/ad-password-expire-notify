...
#--envir adjust--
sed -i.env1bak "s|_ENVIRNAME_|${ENVIRNAME}|g" /usr/local/bin/ad-password-expire-notify/user_email_inlined.tpl
sed -i.env2bak "s|_ENVIRNAME_|${ENVIRNAME}|g" /usr/local/bin/ad-password-expire-notify/admin_email_inlined.tpl
...
#--check_expire adjust--
sed -i.pathbak '0,/\$scriptPath.*/ s/\$scriptPath.*/\$scriptPath = "\/usr\/local\/bin\/ad-password-expire-notify\/";/' /usr/local/bin/ad-password-expire-notify/check_expire.php
sed -i.upnbak "0,/\$ldapupn.*/ s/\$ldapupn.*/\$ldapupn = \"${SVCACCTUPN}\";/" /usr/local/bin/ad-password-expire-notify/check_expire.php
sed -i.passbak "0,/\$ldappass.*/ s/\$ldappass.*/\$ldappass = \"${SVCACCTPASS}\";/" /usr/local/bin/ad-password-expire-notify/check_expire.php
##todo-need to control for $ or other res chars in password
sed -i.hostbak "0,/\$ldaphost.*/ s/\$ldaphost.*/\$ldaphost = \"${LDAPHOSTNAME}\";/" /usr/local/bin/ad-password-expire-notify/check_expire.php
sed -i.mailtobak "0,/\$adminemailto.*/ s/\$adminemailto.*/\$adminemailto = \"${ADMINEMAILTO}\";/" /usr/local/bin/ad-password-expire-notify/check_expire.php
sed -i.usrmailfrombak "0,/\$useremailheader \.= \"From.*/ s/\$useremailheader \.= \"From.*/\$useremailheader \.= \"From: Password Expiration Warning <${EMAILFROM}>\" . \"\\\r\\\n\";/" /usr/local/bin/ad-password-expire-notify/check_expire.php
sed -i.admmailfrombak "0,/\$adminemailheader \.= \"From.*/ s/\$adminemailheader \.= \"From.*/\$adminemailheader \.= \"From: Password Expiration Warning <${EMAILFROM}>\" . \"\\\r\\\n\";/" /usr/local/bin/ad-password-expire-notify/check_expire.php
